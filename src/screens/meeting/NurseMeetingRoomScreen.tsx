...
const NURSE_ACTIONS = ["Appointment Info", "Vitals & Pictures", "Docs"];
...
export function NurseMeetingRoomScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    if (
      appointment?.id &&
      appointment.is_nurse_in_call === false &&
      !hasHungUpRef.current
    ) {
      updateAppointment({
        id: appointment.id as string,
        _set: {
          is_nurse_in_call: true,
        },
      });
    }
  }, [appointment]);
        ...
        <View style={styles.recordingWrap}>
          <Switch
            ...
            onValueChange={(val) => {
              setIsRecording(val);
              sendDataMessage({
                type: val ? "request-recording" : "recording-declined",
              });
            }}
